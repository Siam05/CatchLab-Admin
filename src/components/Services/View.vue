            ...
            <div>
                    ...
                    <DataTable ref="dt" :value="services" stripedRows dataKey="id" :paginator="true" :rows="5" paginatorTemplate="FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink CurrentPageReport RowsPerPageDropdown" :rowsPerPageOptions="[5,10,25]" currentPageReportTemplate="Showing {first} to {last} of {totalRecords} products" responsiveLayout="scroll">
                        ...
                        <Column header="Action" :exportable="false" style="min-width:8rem">
                            <template #body="slotProps">
                                <div class="flex">
                                    <div class="">
                                        <router-link :to="{ path: '/services/edit/' + slotProps.data.slug }">
                                    ...
                                    <div class="ml-2">
                                        <button @click="confirmDeleteService(slotProps.data)" class="bg-red-700 hover:bg-red-800 rounded-full w-9 h-9"><i class="pi pi-trash" style="font-size: 1rem; color: white;"></i></button>
                                    </div>
                                </div>
                            </template>
                        </Column>
                ...
                <Dialog v-model:visible="deleteServiceDialog" :style="{width: '450px'}" header="Confirm" :modal="true">
                    <div class="confirmation-content">
                        ...
                        <span v-if="temp_service">Are you sure you want to delete <b>{{ temp_service.title }}</b>?</span>
                    </div>
                    <template #footer>
                        <Button label="No" icon="pi pi-times" class="p-button-text" @click="deleteServiceDialog = false"/>
                        <Button label="Yes" icon="pi pi-check" class="p-button-text" @click="deleteService(temp_service.id)" />
                    </template>
                </Dialog>
            </div>
        </div>
...
export default {
    ...
    computed: {
        ...mapState ({
            services: state => state.services.service_list
        })
    },

    mounted() {
        this.$store.dispatch('services/getServiceList')
    },

    methods: {
        confirmDeleteService (service) {
            this.temp_service = service;
            this.deleteServiceDialog = true;
        },
        deleteService (id) {
            this.$store.dispatch("services/deleteService", id).then(response => {
